...
struct TotalSpendCard: View {
    ...
    private func card(entries: [(provider: SpendSource, value: Double)]) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            let liveProviders = historySources.compactMap { source -> ProviderID? in
                if case let .provider(provider) = source { return provider }
                return nil
            }
            if !liveProviders.isEmpty {
                LiveUsageStatusView(store: spendHistoryStore, providers: liveProviders)
            }
